<template>
  <el-dialog :title="dialogTitle" v-model="dialogVisible" width="600px" @close="closeDialog" align-center>
    <el-form ref="menuFormRef" :model="menuForm" :rules="formRules" label-width="120px">
      ...
      <!-- 菜单图标 -->
      <el-form-item v-if="setType('M')" label="菜单图标" prop="icon">
      ...
      <!-- 是否外链 -->
      <el-form-item v-if="setType('M') || setType('C')" label="是否外链" prop="isFrame">
      ...
      <!-- 路由地址 -->
      <el-form-item v-if="setType('M') || setType('C')" label="路由地址" prop="path">
        <el-input v-model="menuForm.path" placeholder="请输入路由地址" />
      </el-form-item>
      <!-- 路由组件 -->
      <el-form-item v-if="setType('C')" label="路由组件" prop="component">
        <el-input v-model="menuForm.component" placeholder="请输入路由组件" />
      </el-form-item>
      <!-- 路由参数 -->
      <el-form-item v-if="setType('C')" label="路由参数" prop="query">
        <el-input v-model="menuForm.query" placeholder="请输入路由参数" />
      </el-form-item>
      <!-- 字符权限 -->
      <el-form-item v-if="setType('C') || setType('F')" label="字符权限" prop="perms">
        <el-input v-model="menuForm.perms" placeholder="请输入路由组件" />
      </el-form-item>
      <!-- 是否缓存 -->
      <el-form-item v-if="setType('C')" label="是否缓存" prop="isCache">
      ...
      <!-- 显示状态 -->
      <el-form-item v-if="setType('M') || setType('C')" label="显示状态" prop="visible">
      ...
      <!-- 菜单状态 -->
      <el-form-item v-if="setType('M') || setType('C')" label="菜单状态" prop="status">
...
const dialogTitle = ref(props.status === 'Edit' ? '编辑菜单' : '新增菜单');
